...
class GFootballPolicyValueNet(nn.Module):
    ...
    def warmstart_sparse_from_dense(self):
        with torch.no_grad():
            self.value_fc1_sparse.weight.copy_(self.value_fc1_dense.weight)
            self.value_fc1_sparse.bias.copy_(self.value_fc1_dense.bias)
            self.value_fc2_sparse.weight.copy_(self.value_fc2_dense.weight)
            self.value_fc2_sparse.bias.copy_(self.value_fc2_dense.bias)
        print("Sparse value head warmstarted from dense head!")
...
class CurriculumController:
    ...
    def get_progress_summary(self):
        ...
        for sid in sorted(self.unlocked_stages):
            stats = self.stage_stats[sid]
            status = "*" if sid in self.mastered_stages else ("L" if sid in self.learned_stages else "U")
            if stats['episodes'] > 0:
                recent_wr = np.mean(stats['recent_wins']) if stats['recent_wins'] else 0
...
class ReplayBuffer:
    """Priority replay buffer - fresher + less-used = higher priority."""
    def __init__(self, capacity: int = 50, max_usage: int = 3):
        ...
        self.current_time = 0
    def add(self, trajectories: List[Dict]):
        self.current_time += 1
        for traj in trajectories:
            if len(self.buffer) >= self.capacity:
                scores = [u * 2 + (self.current_time - t) * 0.1 for u, t in zip(self.usage_counts, self.timestamps)]
                idx = int(np.argmax(scores))
                self.buffer.pop(idx)
                self.usage_counts.pop(idx)
    ...
    def sample(self, n: int) -> List[Dict]:
        if not self.buffer:
            return []
        weights = np.array([(1.0 / (1.0 + (self.current_time - t) * 0.1)) * (1.0 / (1.0 + u)) for u, t in zip(self.usage_counts, self.timestamps)])
        weights /= weights.sum()
        indices = np.random.choice(len(self.buffer), size=min(n, len(self.buffer)), replace=False, p=weights)
        samples, expired = [], []
        for idx in indices:
            samples.append(self.buffer[idx])
            self.usage_counts[idx] += 1
            if self.usage_counts[idx] >= self.max_usage:
                expired.append(idx)
        for idx in sorted(expired, reverse=True):
            ...
            self.timestamps.pop(idx)
        return samples
    def get_combined(self, fresh: List[Dict], replay_ratio: float = 0.25) -> List[Dict]:
        self.add(fresh)
        n_replay = max(1, int(len(fresh) * replay_ratio))
        return fresh + self.sample(n_replay)
    def __len__(self):
...
class Learner:
    def __init__(self, config, model_config, writer=None):
        ...
        self.sparse_head_initialized = False
        self.replay_buffer = ReplayBuffer(capacity=50, max_usage=3)
        ...
        print(f"  SI lambda: {config.si_lambda}")
        print(f"  Replay Buffer: capacity=50, max_usage=3, ratio=25%")
    ...
    def _compute_vtrace_batch(self, trajectories):
        ...
        return {'obs': obs, 'features': features, 'actions': actions, 'stage_ids': stage_ids, 'advantages': pg_advantages, 'vtrace_targets': vtrace_targets, 'rho': rho, 'reward_types': reward_types, 'old_log_probs': behavior_log_probs}

    def update(self, trajectories, global_step=0):
        self.model.train()
        all_trajectories = self.replay_buffer.get_combined(trajectories, self.replay_ratio)
        batch = self._compute_vtrace_batch(all_trajectories)
        if batch is None:
            return {}
        ...
        for mb_idx in all_indices:
            ...
            try:
                ...
                policy_loss = -torch.min(ratio * mb_advantages.detach(), clipped_ratio * mb_advantages.detach()).mean()
        ...
        return {'loss/total': total_loss / num_updates, 'loss/policy': policy_loss_sum / num_updates, 'loss/value': value_loss_sum / num_updates, 'loss/entropy': entropy_sum / num_updates, 'vtrace/rho_mean': rho_sum / num_updates, 'vtrace/rho_max': rho.max().item(), 'train/lr': self.optimizer.param_groups[0]['lr'], 'train/nan_count': self.nan_count, 'train/grad_norm': float(grad_norm), 'train/replay_size': len(self.replay_buffer), 'value_heads/dense_samples': total_dense_samples, 'value_heads/sparse_samples': total_sparse_samples, 'value_heads/dense_loss': dense_value_loss_sum / max(1, dense_count), 'value_heads/sparse_loss': sparse_value_loss_sum / max(1, sparse_count)}
    ...
    def update_curriculum_state(self, highest_unlocked_stage: int, frontier_lp: float, frontier_episodes: int):
        ...
        if highest_unlocked_stage >= 10 and not self.sparse_head_initialized:
            self.model.warmstart_sparse_from_dense()
            self.sparse_head_initialized = True
    ...
    def _check_plateau(self, frontier_episodes: int) -> bool:
        ...
        if self.current_lr <= cfg.lr_min:
            return False
        return np.mean(self.lp_history) < cfg.lr_plateau_threshold
    ...
    def _update_lr(self, frontier_episodes: int = 0):
        ...
        if self.update_count < cfg.lr_warmup_steps:
            ...
            for pg in self.optimizer.param_groups:
                pg['lr'] = max(lr, 1e-7)
            return
        if cfg.lr_schedule != "constant" and self._check_plateau(frontier_episodes):
    ...
    def save_checkpoint(self, path, extra=None):
        ckpt = {'model_state_dict': self.model.state_dict(), 'optimizer_state_dict': self.optimizer.state_dict(), 'update_count': self.update_count, 'nan_count': self.nan_count, 'si_omega': {k: v.cpu() for k, v in self.si_omega.items()}, 'si_prev_params': {k: v.cpu() for k, v in self.si_prev_params.items()}, 'feature_importance_history': dict(self.feature_importance_history), 'current_lr': self.current_lr, 'current_highest_stage': self.current_highest_stage, 'lr_reductions_this_stage': self.lr_reductions_this_stage, 'lp_history': list(self.lp_history), 'sparse_head_initialized': self.sparse_head_initialized}
        if extra:
            ckpt.update(extra)
    ...
    def load_checkpoint(self, path):
        ...
        self.sparse_head_initialized = ckpt.get('sparse_head_initialized', self.current_highest_stage >= 10)
        old_lr = ckpt.get('current_lr', 'unknown')
        self.current_lr = self.config.learning_rate
        print(f"  LR RESET: {old_lr} -> {self.current_lr:.2e}")
        for pg in self.optimizer.param_groups:
            pg['lr'] = self.current_lr
        return ckpt
...
class IMPALATrainer:
    ...
    def train(self):
        ...
        while True:
            ...
            if num_learned > self.last_highest_unlocked:
                ...
                print(f"\nNo progress for {self.config.max_steps_without_progress:,} steps")
                break
    ...
    def _log_progress(self, update_count, stats):
        ...
        replay_size = stats.get('train/replay_size', 0)
        print(f"[{update_count}] {self.total_steps / 1e6:.1f}M | {sps / 1e3:.0f}k sps | LR:{lr:.1e} | Loss:{stats.get('loss/total', 0):.3f} | RB:{replay_size}")
        ...
        if stage_stats:
            ...
            for sid in sorted(curriculum_stats.get('unlocked_stages', [0])):
                if str(sid) in stage_stats and stage_stats[str(sid)]['episodes'] > 0:
                    ...
                    marker = "*" if sid in mastered else ("L" if sid in learned else "")
                    frontier.append(f"S{sid}:{wr:.0%}{marker}")
            print(f"{' | '.join(frontier)}")
        if self.writer:
            self.writer.add_scalar('train/lr', lr, self.total_steps)
            self.writer.add_scalar('train/replay_size', replay_size, self.total_steps)
