...
class MambaBlock(nn.Module):
    def __init__(self, d_model, d_state=128, segment_size=8):
        super().__init__()
    ...
    def forward(self, x, h):
        ...
        return x + self.out_proj(out), h.view(B, -1)

class MambaEncoder(nn.Module):
    def __init__(self, input_dim, d_model=256, d_state=128, num_layers=8, segment_size=8):
        super().__init__()
        self.d_model, self.d_state, self.num_layers = d_model, d_state, num_layers
        self.output_dim = d_model
        self.input_proj = nn.Linear(input_dim, d_model)
        self.layers = nn.ModuleList([MambaBlock(d_model, d_state, segment_size) for _ in range(num_layers)])
...
class GFootballPolicyValueNet(nn.Module):
    def __init__(self, config):
        ...
        self.mamba = MambaEncoder(input_dim=in_dim, d_model=config.d_model, d_state=config.mamba_d_state, num_layers=config.mamba_layers, segment_size=config.segment_size)
        policy_layers = []
...
class Learner:
    def __init__(self, config, model_config, writer=None):
        ...
        self.model = create_model(model_config)
        self.model.to(self.device)
        self.use_prodigy = getattr(config, 'use_prodigy', False)
        if self.use_prodigy:
            self.optimizer = Prodigy(self.model.parameters(), lr=1.0, d_coef=getattr(config, 'prodigy_d_coef', 1.0), weight_decay=config.weight_decay, safeguard_warmup=True, use_bias_correction=True)
        else:
            self.optimizer = torch.optim.AdamW(self.model.parameters(), lr=config.learning_rate, eps=1e-5, weight_decay=config.weight_decay)
        ...
        print(f"  Optimizer: {'Prodigy' if self.use_prodigy else 'AdamW'}")
    ...
    def _get_prodigy_d(self):
        if hasattr(self.optimizer, 'param_groups'):
            return self.optimizer.param_groups[0].get('d', 1.0)
        return 1.0
    ...
    def update(self, trajectories, global_step=0):
        ...
        result = {'loss/total': total_loss / num_updates, 'loss/policy': policy_loss_sum / num_updates, 'loss/value': value_loss_sum / num_updates, 'loss/entropy': entropy_sum / num_updates, 'vtrace/rho_mean': rho_sum / num_updates, 'vtrace/rho_max': rho.max().item(), 'vtrace/explained_variance': explained_var, 'train/nan_count': self.nan_count, 'train/grad_norm': float(grad_norm), 'value_heads/dense_samples': total_dense_samples, 'value_heads/sparse_samples': total_sparse_samples}
        if self.use_prodigy:
            result['train/prodigy_d'] = self._get_prodigy_d()
        else:
            result['train/lr'] = self.optimizer.param_groups[0]['lr']
        if dense_count > 0:
    ...
    def update_curriculum_state(self, highest_unlocked_stage: int, frontier_lp: float, frontier_episodes: int):
        if highest_unlocked_stage > self.current_highest_stage:
            ...
            if self.use_prodigy:
                print(f"\n🔓 Stage {highest_unlocked_stage} unlocked - D: {self._get_prodigy_d():.2e}\n")
            else:
                print(f"\n🔓 Stage {highest_unlocked_stage} unlocked\n")
    def save_checkpoint(self, path, extra=None):
        ckpt = {'model_state_dict': self.model.state_dict(), 'optimizer_state_dict': self.optimizer.state_dict(), 'update_count': self.update_count, 'nan_count': self.nan_count, 'si_omega': {k: v.cpu() for k, v in self.si_omega.items()}, 'si_prev_params': {k: v.cpu() for k, v in self.si_prev_params.items()}, 'feature_importance_history': dict(self.feature_importance_history), 'current_highest_stage': self.current_highest_stage, 'use_prodigy': self.use_prodigy, 'ema_weights': {k: v.cpu() for k, v in self.ema_weights.items()} if self.ema_enabled else None}
        if extra:
            ckpt.update(extra)
...
class IMPALATrainer:
    ...
    def setup(self):
        print("=" * 60)
        print(f"IMPALA TRAINER - PRODIGY CONFIG")
        print(f"MODEL: d_model=128, d_state=32, layers=6 (~1M params)")
        print(f"SCALE: {self.config.num_workers} workers, batch={self.config.batch_size}")
        print(f"OPTIMIZER: Prodigy (d_coef={self.config.prodigy_d_coef})")
        if self.resume_from:
            print(f"RESUMING FROM: {self.resume_from}")
        print("=" * 60)
        run_name = f"gfootball_prodigy_{time.strftime('%Y%m%d_%H%M%S')}"
        self.writer = SummaryWriter(log_dir=self.log_dir / run_name)
        if not self.calibrator.load():
            self.calibrator.calibrate(num_episodes=100, num_workers=min(self.config.num_workers, 32))
        ...
        print(f"Spawning {self.config.num_workers} workers...")
        ...
        print(f"Syncing weights to {self.config.num_workers} workers...")
        self._sync_weights()
    ...
    def train(self):
        ...
        while True:
            ...
            if num_learned > self.last_highest_unlocked:
                ...
                break
            done_refs, _ = ray.wait(list(pending.keys()), num_returns=min(8, len(pending)))
    ...
    def _log_progress(self, update_count, stats):
        ...
        if self.learner.use_prodigy:
            d_val = stats.get('train/prodigy_d', 1.0)
            print(f"[{update_count}] {self.total_steps / 1e6:.1f}M | {sps / 1e3:.0f}k sps | D:{d_val:.2e} | ρ={rho_mean:.2f} | Loss:{stats.get('loss/total', 0):.3f}")
            if self.writer:
                self.writer.add_scalar('train/prodigy_d', d_val, self.total_steps)
        else:
            lr = stats.get('train/lr', 0)
            print(f"[{update_count}] {self.total_steps / 1e6:.1f}M | {sps / 1e3:.0f}k sps | LR:{lr:.1e} | ρ={rho_mean:.2f} | Loss:{stats.get('loss/total', 0):.3f}")
            if self.writer:
        ...
        if self.writer:
            self.writer.add_scalar('train/sps', sps, self.total_steps)
            self.writer.add_scalar('vtrace/rho_mean', rho_mean, self.total_steps)
...
def main():
    RESUME_FROM = None
    CHECKPOINT_DIR = "./checkpoints_prodigy"
    LOG_DIR = "./logs_prodigy"
    model_config = {'d_model': 128, 'mamba_d_state': 32, 'mamba_layers': 6, 'encoder_hidden': [128], 'policy_hidden': [128], 'value_hidden': [128], 'action_emb_dim': 16, 'stage_emb_dim': 8, 'segment_size': 4}
    config = TrainingConfig(stages=get_default_stages(), final_stage_target_win_rate=0.5, max_steps_without_progress=100_000_000, num_workers=256, trajectory_length=128, batch_size=4096, minibatch_size=256, num_epochs=2, use_prodigy=True, prodigy_d_coef=1.0, learning_rate=1.0, weight_decay=0.01, gamma=1.0, rho_bar=1.0, c_bar=1.0, entropy_coeff=0.01, value_coeff=0.5, si_lambda=1.0, max_grad_norm=0.5, total_steps=1_000_000_000, log_interval=10, checkpoint_interval=100, feature_importance_interval=50, weight_sync_interval=10, log_dir=LOG_DIR, checkpoint_dir=CHECKPOINT_DIR)
    test_model = create_model(model_config)
    total_params = count_parameters(test_model)
    print(f"\n{'=' * 60}")
    print(f"PRODIGY CONFIGURATION")
    print(f"{'=' * 60}")
    print(f"MODEL PARAMETERS: {total_params:,}")
    print(f"  Encoder: {sum(p.numel() for p in test_model.obs_encoder.parameters()):,}")
    print(f"  Mamba: {sum(p.numel() for p in test_model.mamba.parameters()):,}")
    print(f"  Policy: {sum(p.numel() for p in test_model.policy_head.parameters()):,}")
    print(f"TRAINING SCALE:")
    print(f"  Workers: {config.num_workers}")
    print(f"  Batch: {config.batch_size}")
    print(f"  Minibatch: {config.minibatch_size}")
    print(f"  Prodigy d_coef: {config.prodigy_d_coef}")
    print(f"{'=' * 60}\n")
    del test_model
