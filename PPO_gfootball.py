...
class GFootballMultiAgentEnv(MultiAgentEnv):
    def __init__(self, config: Dict[str, Any]):
        ...
        try:
            ...
            _initial_obs_sample = (_initial_obs_sample[0] if isinstance(_initial_obs_sample, tuple)
                                    else _initial_obs_sample)

            if not self.agent_ids:
                single_agent_obs_shape = _initial_obs_sample.shape
            elif (_initial_obs_sample.ndim > 0 and
                    _initial_obs_sample.shape[0] == (self.left_players + self.right_players) and
                    (self.left_players + self.right_players) > 0):
    ...
    def step(self, action_dict):
        ...
        return (self.latest_obs, self._split_rewards(rewards),
                dones, truncs, agent_infos)
    ...
    def _split_obs(self, obs):
        ...
        elif (obs.ndim == 1 and num_agents > 0 and
                self.observation_space.shape is not None and
                np.prod(self.observation_space.shape) > 0 and
                obs.size == num_agents * int(np.prod(self.observation_space.shape))):
            obs_reshaped = obs.reshape(num_agents, *self.observation_space.shape)
            return {self.agent_ids[i]: obs_reshaped[i].astype(np.float32)
    ...
    def _split_rewards(self, rewards):
        ...
        if isinstance(rewards, (list, np.ndarray)):
            if len(rewards) == len(self.agent_ids):
                return {self.agent_ids[i]: float(rewards[i])
...
def main():
    ...
    if best_result:
        best_checkpoint = best_result.get_best_checkpoint(metric="env_runners/episode_return_mean", mode="max")
        print(f"Bestes Ergebnis erreicht:")
        print(f"  Return Mean: {best_result.metrics['env_runners']['episode_return_mean']}")
