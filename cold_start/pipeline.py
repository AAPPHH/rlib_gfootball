    ...
    def _extract_simple115_features(self, replay_data: Dict, replay_id: str) -> Tuple[List, List, List]:
        ...
        try:
            ...
            for step_idx, step in enumerate(steps):
                ...
                all_features = (
                    ball_features + ball_dir_features + left_pos + left_vel + 
                    right_pos + right_vel + game_mode_features + 
                    ball_owned_features + active_features
                )
    ...
    def build_lake(self):
        ...
        try:
            ...
            conn.execute("""
                ...
                SELECT a.*, o.global_idx
                FROM act_df a
                JOIN observations o ON a.replay_id = o.replay_id AND a.step = o.step
            """)
            ...
            conn.execute("""
                ...
                SELECT r.*, o.global_idx
                FROM rew_df r
                JOIN observations o ON r.replay_id = o.replay_id AND r.step = o.step
            """)
                ...
                if n_base_features == 0:
                    ...
                    for offset in range(self.stack_frames):
                        for col in feature_cols:
                            if offset == 0:
                                lag_clauses.append(f"{col} AS {col}_lag0")
                            else:
                                lag_clause = (
                                    f"COALESCE("
