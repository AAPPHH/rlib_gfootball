"""
Step 2: XGBoost Hyperparameter Tuning with Full Dataset Memorization
...
class XGBoostMemorizationTuner:
    def __init__(self, ducklake_path: str, output_dir: str = "./xgboost_tuned", stack_frames: int = 4):
        ...
        logger.info("="*70)
        logger.info("XGBoost Memorization Tuner")
        logger.info("="*70)
    ...
    def load_full_dataset(self):
            ...
            try:
                ...
                # --- KORREKTUR: "filename" aus der EXCLUDE-Liste entfernt ---
                query = f"""
                    SELECT 
    ...
    def tune_hyperparameters(self, X, y, n_trials=100):
        ...
        logger.info("Using 3-fold CV for robust hyperparameter selection\n")
    ...
    def train_final_model(self, X, y, params):
        ...
        if final_params['n_estimators'] < 1000:
            final_params['n_estimators'] = 1000
            logger.info(f"Increased n_estimators to {final_params['n_estimators']} for better memorization")
        
        logger.info("\nFinal parameters:")
        for key, value in final_params.items():
            if key not in ['tree_method', 'device', 'num_class', 'objective', 'eval_metric', 'random_state', 'verbosity', 'n_jobs']:
                logger.info(f"  {key}: {value}")
        ...
        if train_accuracy < 0.99:
            logger.warning(f"⚠️ Accuracy below 99% - consider more iterations or deeper trees")
        else:
