...
def create_impala_config(
    ...
    config.resources(num_cpus_for_main_process=1, num_gpus=0)
    config.learners(
        num_learners=1,
    ...
    if has_left or has_right:
        config.callbacks(
            lambda: EnhancedSelfPlayCallback(
                update_interval=50,
        ...
        if restore_path:
            try:
                ...
                if hasattr(self.algo, "_counters"):
                    ...
                    self.start_timesteps = dummy_result.get("timesteps_total", 0) - dummy_result.get(
...
def train_stage_sequential_pbt(
    ...
    while gen < generations:
        ...
        resources_per_trial = tune.PlacementGroupFactory(
            [{"CPU": 1}, {"CPU": 1, "GPU": tune_config["gpu_per_trial"]}]
...
def main():
    ...
    debug_mode = False
    initial_checkpoint = (
        r"/home/john/rlib_gfootball/training_results_transfer_2/"
        r"stage_2_basic_pbt_parallel_20251113_110913/"
        r"train_impala_with_restore_ce0df_lr=2.5e-05_ent=0.009_vf=0.92/"
        r"checkpoint_001023"
    )
    ...
    for i in range(start_stage_index, end_stage_index + 1):
        ...
        stage_tb_cmd = (
            f'tensorboard --logdir "{stage_root}" '
            f'--reload_multifile true --purge_orphaned_data true '
            f'--window_title "{stage.name} - All Generations"'
        )
        log_file = results_path / "tensorboard_commands.txt"
        try:
            with open(log_file, "a", encoding="utf-8") as f:
